...
## Data Structure
Files follow a naming convention: subject_[ID]/Syn3DWound_[Wound_ID]_[body_part]_[lighting]_[angle]
```text
Syn3DWound/
...
- **Syn3DObject folders**: Contain 3D reconstructions (likely .phi/.theta files for angular views), suitable for NeRF or SDF training.[web:7]
- **.obj/.txt files**: Mesh models and text annotations (e.g., vertices, bounding boxes).
- **images/**: RGB images of wounds from multiple viewpoints.
- **mask folders**: Binary or semantic masks for wound segmentation under specific conditions.[web:13]

| File Type | Description | Example Use |
|-----------|-------------|-------------|
| Syn3DObject/*.phi | 3D model (phi angle) | Reconstruction input [web:16] |
| images/*.jpg | Multi-view photos | Training 2D/3D models |
| mask_*.phi/*.colmap | COLMAP masks (strong lighting) | Segmentation ground truth [web:10] |
| BB.txt | Bounding boxes | Object detection |
